...
var ALL_MODULES = [
  { id: 'plug', label: '插头线', icon: '\u{1f50c}' },
  { id: 'pkg', label: '包装', icon: '\u{1f4e6}' },
  { id: 'alu', label: '铝型材', icon: '\u{1f529}' },
  { id: 'plastic', label: '塑料件', icon: '\u{1fa79}' },
  { id: 'pcb', label: 'PCB', icon: '\u{1f4df}' }
];

function getUserModules(user) {
  if (!user || user.role === 'admin') return ALL_MODULES.map(function(m) { return m.id; });
  if (user.modules && user.modules.length) return user.modules;
  return ALL_MODULES.map(function(m) { return m.id; });
}

function canAccessModule(mid) {
  return getUserModules(getCurrentUser()).indexOf(mid) >= 0;
}
...
function markModuleChange(uid, mid, chk) {
  if (!_modPending[uid]) _modPending[uid] = {};
  _modPending[uid][mid] = chk;
}
...
function saveAllModules() {
  ...
  cbs.forEach(function(cb) {
    ...
    if (!oc) return;
    var mt = oc.match(/markModuleChange\('(.+?)','(.+?)'/);
    if (!mt) return;
    var uid = mt[1], mid = mt[2];
    if (!userMs[uid]) userMs[uid] = [];
    if (cb.checked) userMs[uid].push(mid);
  });
  var uids = Object.keys(userMs);
  if (uids.length === 0) { alert('没有待保存的更改'); return; }
  var ps = uids.map(function(uid) {
    return supabaseUpdate('app_users', { id: uid }, { modules: userMs[uid] });
  });
...
function buildModuleCheckboxes(u) {
  var mods = u.modules || [];
  var h = '';
  ALL_MODULES.forEach(function(md) {
    var checked = (mods.length === 0 || mods.indexOf(md.id) >= 0) ? 'checked' : '';
    h += '<label style="display:inline-block;margin:0 2px;cursor:pointer;font-size:11px" title="' + md.label + '"><input type="checkbox" ' + checked + ' onchange="markModuleChange(\'' + u.id + '\',\'' + md.id + '\',this.checked)">' + md.icon + '</label>';
  });
  return h + '<br><span style="color:#94A3B8;font-size:9px">点击勾选保存</span>';
}
...
function renderAdminUserList() {
  ...
  var html = statsHtml + '<table style="width:100%;border-collapse:collapse;font-size:12px;">' +
    ...
    '<th style="padding:6px 4px;">模块</th>' +
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    html += '<tr style="border-bottom:1px solid #F1F5F9;' + (isMe ? 'background:#EFF6FF;' : '') + '">' +
      ...
      '<td style="padding:6px 4px;font-size:10px;max-width:120px;white-space:normal;line-height:1.4;">' + buildModuleCheckboxes(u) + '</td>' +
...
async function adminChangeRole(userId, newRole) {
  if (!confirm('确认将用户角色改为' + (newRole === 'admin' ? '管理员' : '普通用户') + '？')) return;
  try {
